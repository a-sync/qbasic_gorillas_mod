...
SlidyText:
DATA 5
DATA "      Q B a s i c  G O R I L L A S  v2.2 (Vectors mod)",15,1,4
...
SUB GorillaIntro (Player$(), cIntro)
  IF cIntro = 1 THEN 'þ cIntro = 0 means introduction compulsory
   ...
   COLOR 9: LOCATE 19, 32: PRINT "Any other key to play..."
   LOCATE 17, 34: PRINT "= View Intro"
   LOCATE 18, 34: PRINT "= Quit Gorillas"
   LOCATE 21, 34: PRINT "Your Choice?"
   COLOR 12: LOCATE 17, 32: PRINT "V": 'LOCATE 19, 32: PRINT "P"
   LOCATE 18, 32: PRINT "Q"
   DO
    Char$ = UCASE$(INKEY$)
...
SUB MakeCityScape (BCoor() AS XYPoint)
  ...
  IF Mode = 9 THEN
    ...
    DefBWidth = 28                     'Default building width
    RandomHeight = 120                 'Random height difference
    WWidth = 3                         'Window width
    WHeight = 5                        'Window height
    WDifV = 12                         'Counter for window spacing - vertical
...
FUNCTION PlayGame (Player$(), NumGames, P)
 ...
 DO
  ...
  DO WHILE Hit = FALSE
   ...
   PRINT Player$(2);
   IF Mode = 9 THEN COLOR 9
   Center 23, STR$(totalWins(1)) + " : " + LTRIM$(STR$(totalWins(2)) + " ")
...
SUB Stats (Wins(), nam$(), Ban!(), P, abortYN)
 ...
 ELSE
  ...
  FOR l = 1 TO 2
   ...
   IF (Wins(1) <> Wins(2)) THEN
     IF D = -1 THEN PRINT CHR$(27); "ÄÄÄÄ Winner!";
   ELSEIF l = 1 THEN
